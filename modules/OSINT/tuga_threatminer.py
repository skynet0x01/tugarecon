# --------------------------------------------------------------------------------------------------
# TugaRecon – Threatminer OSINT Module
...
class Threatminer:
    ...
    def engine_url(self):
        ...
        try:
            ...
            if r.status_code != 200:
                log.debug(f"[Threatminer] HTTP {r.status_code} for {self.target}")
                return None
            return r.text
        except (requests.ConnectionError, requests.Timeout) as e:
            log.debug(f"[Threatminer] Request failed: {e}")
            return None
    ...
    def enumerate(self, response_text):
        ...
        try:
            ...
            #log.info(f"[Threatminer] {self.subdomainscount} unique subdomains found in {elapsed}s")

        except Exception as e:
            log.debug(f"[Threatminer] Parsing error: {e}")
