...
# File: modules/Brute_force/tuga_bruteforce.py
...
class TugaBruteForce:
    ...
    def __init__(self, options: dict):
        ...
        # Semantic hints (OSINT + contextual words)
        ...
        # ⚠️ IMPORTANTE: inicializar cache ANTES de wildcard
    ...
    def print_testing(self, domain):
        now = time.time()
        if not hasattr(self, "last_spinner_update"):
    ...
    def scan_subdomains(self):
        try:
            while True:
                ...
                if self.subdomains_count.value % 20 == 0:
...
# # --------------------------------------------------------------------------------------------------
# # TugaRecon – Brute-force Module (Optimized Phase A)
# # Author: Skynet0x01 2020-2026
# # GitHub: https://github.com/skynet0x01/tugarecon
# # License: GNU GPLv3
# # Patent Restriction Notice:
...
# class TugaBruteForce:
#     SPINNER = ['|', '/', '-', '\\']
#     BUFFER_SIZE = 50  # write to disk every N valid results
#
#     def __init__(self, options: dict):
#         freeze_support()
#
...
#         self.file_lock = Lock()
#
#         # Buffer for batch writing results (reduces disk I/O overhead)
#         self.write_buffer = []
#         self.buffer_lock = Lock()
#
#         # Cache of domains known to NOT exist (NXDOMAIN), avoids useless re-queries
...
#         self.spinner_index = 0
#
#         # Initialize resolvers pool (one resolver per DNS server)
#         self.resolvers = []
#         for server in self.online_dns_servers:
#             r = dns.resolver.Resolver(configure=False)
#             r.nameservers = [server]
...
#             self.resolvers.append(r)
#
#         # Detect wildcard DNS (to ignore fake positives)
#         self.wildcard_ips = self.detect_wildcard() or set()
#
#     # ----------------------------------------------------------------------------------------------------------
#     # Load first-level subdomain wordlist + semantic hints
#
#     def load_first_sub_names(self):
#         seen = set()
#
...
#
#     # ----------------------------------------------------------------------------------------------------------
#     # Load second-level names (for nested subdomains)
#
#     def load_second_sub_names(self):
#         if not os.path.exists(self.second_wordlist):
#             return []
...
#
#     # ----------------------------------------------------------------------------------------------------------
#     # Test which DNS servers are online
#
#     def get_online_dns_servers(self, test_domain="google.com"):
#         online = []
#         for server in self.dns_servers:
...
#
#     def resolve_fast(self, domain):
#         # Skip if already known as NXDOMAIN
...
#                 # Domain does not exist → cache and stop retrying
#                 self.negative_cache.add(domain)
#                 return None
#
#             except dns.resolver.Timeout:
#                 # Try next resolver
...
#
#         if last_error:
#             self.log_error(domain, last_error)
#
#         return None
#
#     # ----------------------------------------------------------------------------------------------------------
...
#
#     # ----------------------------------------------------------------------------------------------------------
#     # Buffered write handler (batch disk I/O)
#
...
#         self.print_valid(count, domain, ips)
#
#         # Buffer instead of writing immediately (faster)
#         with self.buffer_lock:
#             self.write_buffer.append(f"{domain}\t{','.join(ips)}\n")
...
#                     self.handle_valid_subdomain(sub1, ips)
#
#                     # Nested subdomains (second level)
#                     for second in self.second_sub_names:
#                         sub2 = f"{second}.{sub1}"
